           ...
           result[0].comment.forEach(function(res)
           {
               ...
               request(author_link_data, function (error, response, html) {
                ...
                var chId = author_link_data;
                 ...
                 let related_channels_title = $('.yt-ui-ellipsis-2').attr('title',(j,val) =>{ arr[chId]['title'].push(val);}); 
                 let related_channels_link = $('.yt-ui-ellipsis-2').attr('href',(j,val) =>{ arr[chId]['link'].push(val); });
                 ...
                 var link_obj= JSON.stringify(arr[chId]['link']);
                  ...
                  if(title_obj != '[]')
                  {
                  const child = execFileSync('node', ['./lib/channel/sub_insert_channel_db.js', channel_id , title_obj ,link_obj ])
                 }
              })
